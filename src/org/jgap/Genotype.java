...
public MarioConfiguration marioConf;
public EvaluationInfo maxEval;
public EvaluationInfo minEval;
public EvaluationInfo avgEval;
...
protected void addChromosomesFromMaterial( Collection chromosomeMaterial ) {
	...
	while ( iter.hasNext() ) {
		...
		{
			//EHSAN try to fix this error
			System.out.println("Try To fix this error");
			e.printStackTrace();
		}
	}
}
 ...
 * @see Genotype#addChromosome(Chromosome)
 */
protected void addChromosomeFromMaterial( ChromosomeMaterial cMat ) throws ClassCastException {
...
public synchronized void evolve() {
	try {
		...
		MarioFitness marioBulkFunction = (MarioFitness)bulkFunction;
		if ( bulkFunction != null )
		{
			marioBulkFunction.config = this.marioConf;
			...
			// now.
			//EHSAN   create the fitness function program and mario runs inside that function.  
		...
		// EHSAN SHOW THE BEST CHROMOSOME
		marioBulkFunction = (MarioFitness)bulkFunction;
		if(marioBulkFunction.config.isChampVisualisation())
		{
			
			marioBulkFunction.config.setVisOption(marioBulkFunction.config.isChampVisualisation());
		}
		List<Chromosome> chromosomes = this.getChromosomes();
		Iterator<Chromosome> Testiterator= chromosomes.iterator();
		Chromosome chrom =null;
		int chromNumber=0;
		while(Testiterator.hasNext())
		{
			chrom = Testiterator.next();
			if(chrom.getFitnessValue()>=1000)
			{
				marioBulkFunction.config.configPassedtoNextMario();
				for(int i=0;i<marioBulkFunction.config.totalLevelPerTest;i++)
				{
				
					if(chrom.champIndex==-1)
						marioBulkFunction.config.setRecording("Champ"+"NormalFitness"+" Test"+i);
					else
					{
						marioBulkFunction.config.setRecording("Champ"+MarioPlanetSystemOfValues.variableNames[chrom.champIndex]+" Test"+i);
						
					}
				
					try {
						if(chrom.champIndex==-1)
							marioBulkFunction.runMario(this.getFittestChromosome());
						else
							marioBulkFunction.runMario(chrom);
						System.out.println(chrom.getFitnessValue());
		...
		marioBulkFunction.config.setRecording("off");
		...
		System.out.println("Genotype: End of Genotype Evolve");
	}
